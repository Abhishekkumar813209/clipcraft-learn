...
import { RBI_ALL_TOPICS } from '@/types/rbi';

export function useRbiTodayStats() {
  const { user } = useAuth();
  return useQuery({
    queryKey: ['rbi-today-stats', user?.id],
    enabled: !!user,
    queryFn: async () => {
...
export function useRbiTopicAccuracy() {
  const { user } = useAuth();
  return useQuery({
    queryKey: ['rbi-topic-accuracy', user?.id],
    enabled: !!user,
    queryFn: async () => {
      const { data } = await supabase
      ...
      (data || []).forEach((p: any) => {
        ...
        if (!topic || !RBI_ALL_TOPICS.includes(topic)) return;
...
export function useRbiSubmitAnswer() {
  const { user } = useAuth();
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['rbi-today-stats'] });
      qc.invalidateQueries({ queryKey: ['rbi-topic-accuracy'] });
    },
  });
}
