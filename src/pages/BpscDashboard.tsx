import { useBpscTodayStats, useBpscTopicAccuracy } from '@/hooks/useBpscProgress';
import { useBpscQuestionCount } from '@/hooks/useBpscQuestions';
import { BPSC_TOPIC_META, BPSC_ALL_TOPICS, BPSC_SUBJECTS, BPSC_SUBJECT_TOPICS } from '@/types/bpsc';
...
export default function BpscDashboard() {
  const { data: stats } = useBpscTodayStats();
  const { data: topicAccuracy } = useBpscTopicAccuracy();
  const { data: questionCounts } = useBpscQuestionCount();
  ...
  const weakTopics = BPSC_ALL_TOPICS.filter((t) => {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">BPSC Dashboard 🏛️</h1>
        <p className="text-muted-foreground">Track your Bihar PSC exam preparation progress.</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard icon={<Target className="h-5 w-5 text-emerald-600" />} label="Today's Goal" value={`${solved}/${DAILY_GOAL}`} sub={`${goalPct}% done`} />
      ...
      {BPSC_SUBJECTS.map((subject) => {
        const subTopics = BPSC_SUBJECT_TOPICS[subject.key];
        return (
                ...
                {subTopics.map((t) => {
                  ...
                  return (
                    <div key={t} className="flex items-center gap-2 p-2 rounded-lg bg-accent/30">
                      <span className="text-lg">{BPSC_TOPIC_META[t].icon}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium truncate">{BPSC_TOPIC_META[t].label}</p>
