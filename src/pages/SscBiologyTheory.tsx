...
export default function SscBiologyTheory() {
  ...
  const practiceHref = `/ssc/gk/biology/practice?chapter=${encodeURIComponent(chapter)}${subtopic ? `&subtopic=${encodeURIComponent(subtopic)}` : ''}`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 text-slate-900">
      <div className="sticky top-0 z-10 backdrop-blur bg-white/80 border-b border-emerald-100">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          <Button size="sm" variant="ghost" className="text-emerald-700" onClick={() => nav('/ssc/gk/biology')}>
            <ArrowLeft className="w-4 h-4 mr-1" /> Back
          </Button>
          <div className="min-w-0 flex-1">
            <div className="text-sm font-semibold truncate">{subtopic || chapter}</div>
            <div className="text-[11px] text-slate-500 hidden sm:block">
              Biology · Hinglish theory {row ? `· ${row.question_count} questions se bani` : ''}
            </div>
          </div>
