              ...
              <Card key={g.key} className={`bg-gradient-to-br ${g.color} border-emerald-100 shadow-sm`}>
                <CardContent className="p-5 space-y-3">
                  ...
                  <div className="flex gap-2 pt-1">
                    ...
                    <Link to={`/ssc/blackbook/practice/idiom?sub=${g.key}&n=${validN}&order=${orderMap[g.key]}&diff=${diffMap[g.key]}${fromMap[g.key] ? `&from=${fromMap[g.key]}` : ''}${toMap[g.key] ? `&to=${toMap[g.key]}` : ''}`} className="flex-1">
