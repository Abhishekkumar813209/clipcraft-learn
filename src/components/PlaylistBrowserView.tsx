...
export function PlaylistBrowserView() {
  ...
  useEffect(() => {
    if (!searchQuery.trim()) {
      ...
      setFilteredVideos(videos.filter(v => v.title.toLowerCase().includes(query) || v.channelName.toLowerCase().includes(query)));
    }
  }, [searchQuery, videos]);
    ...
    <div className="flex-1 overflow-auto p-8">
      <div className="flex items-center gap-4 mb-8 animate-fade-in">
        ...
        <div className="flex-1">
          <h1 className="font-display text-2xl font-bold">{source?.title || 'Playlist Browser'}</h1>
      ...
      <div className="relative mb-6">
        ...
        <Input placeholder="Search videos..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 bg-background border-input" />
      </div>
      ...
      {!loading && !error && (
        <div className="space-y-3">
          {filteredVideos.map((video, index) => (
            ...
            >
              <div className="relative flex-shrink-0 w-40 aspect-video rounded-lg overflow-hidden bg-secondary">
                ...
                <div className="absolute bottom-1 right-1 bg-black/80 text-white text-xs px-1.5 py-0.5 rounded">{formatDuration(video.duration)}</div>
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-1">#{video.position + 1} in playlist</p>
              </div>
          ...
          {filteredVideos.length === 0 && videos.length > 0 && (
            <div className="text-center py-8"><p className="text-muted-foreground">No videos match your search</p></div>
          )}
        </div>
      )}
    </div>
