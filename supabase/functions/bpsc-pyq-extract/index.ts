...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an expert at parsing BPSC (Bihar Public Service Commission) exam question papers. Extract ALL multiple-choice questions (MCQs) from the given text.
...
4. Classify into one of these BPSC topics: ${BPSC_TOPICS.join(", ")}
...
- If you cannot determine the correct answer, set correct_option to 0 and note it in the explanation`;
    ...
    console.error("bpsc-pyq-extract error:", e);
    return new Response(
