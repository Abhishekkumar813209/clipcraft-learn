...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an expert at parsing RBI Grade B exam question papers. Extract ALL multiple-choice questions (MCQs) from the given text.
...
4. Classify into one of these RBI topics: ${RBI_TOPICS.join(", ")}
...
- If you cannot determine the correct answer, set correct_option to 0 and note it in the explanation`;
    ...
    const response = await fetch(AI_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `Extract all MCQ questions from this RBI Grade B ${year || ""} exam paper text:\n\n${pageText}` },
        ],
        tools: [{
          type: "function",
          function: {
                ...
                questions: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      topic: { type: "string", enum: RBI_TOPICS, description: "RBI topic classification" },
    ...
    console.error("rbi-pyq-extract error:", e);
    return new Response(
